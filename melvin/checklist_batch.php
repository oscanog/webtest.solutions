...
<?php if ($batch): ?>
    ...
    <div class="bc-panel" data-checklist-table>
                ...
                <?php else: ?>
                    <?php foreach ($items as $item): ?>
                        ...
                        <tr data-checklist-row data-status="<?= bugcatcher_html((string) $item['status']) ?>" data-assignment="<?= bugcatcher_html($assigneeName !== '' ? 'assigned' : 'unassigned') ?>" data-search-base="<?= bugcatcher_html($searchBase) ?>" data-search-assignee="<?= bugcatcher_html(strtolower($assigneeName)) ?>">
                            ...
                            <td><?php if (!empty($item['issue_id'])): ?><a href="<?= bugcatcher_html(bugcatcher_path('zen/dashboard.php?page=dashboard&status=open')) ?>">#<?= (int) $item['issue_id'] ?></a><?php else: ?><span class="bc-meta">None</span><?php endif; ?></td>
                            <td data-checklist-updated-cell><?= bugcatcher_html(bugcatcher_checklist_format_datetime($item['updated_at'] ?: $item['created_at'])) ?></td>
                            <?php if ($isChecklistManager): ?>
                                <td><form class="bc-assignment-form" method="post" data-checklist-assignment-form data-endpoint="<?= bugcatcher_html($assignmentEndpointBase . '?id=' . (int) $item['id']) ?>"><label class="bc-screen-reader" for="assigned_to_user_id_<?= (int) $item['id'] ?>">QA Tester</label><select class="bc-select bc-assignment-select" id="assigned_to_user_id_<?= (int) $item['id'] ?>" name="assigned_to_user_id"><option value="0">Unassigned</option><?php foreach ($qaTesters as $qaTester): ?><option value="<?= (int) $qaTester['id'] ?>" <?= (int) $item['assigned_to_user_id'] === (int) $qaTester['id'] ? 'selected' : '' ?>><?= bugcatcher_html($qaTester['username']) ?></option><?php endforeach; ?></select><div class="bc-assignment-actions"><button type="submit" class="bc-btn secondary" data-checklist-apply>Apply</button><button type="button" class="bc-btn secondary" data-checklist-clear>Clear</button></div><span class="bc-meta bc-form-status" data-checklist-form-status aria-live="polite"></span></form></td>
                            <?php endif; ?>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
    ...
    <script src="<?= bugcatcher_html(bugcatcher_path('app/checklist_batch_page.js?v=1')) ?>"></script>
<?php endif; ?>
