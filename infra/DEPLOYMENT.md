...
- Put your production config at `/var/www/webtest/shared/config.php`
- Keep uploads in `/var/www/webtest/shared/uploads/issues`
- Keep checklist uploads in `/var/www/webtest/shared/uploads/checklists`
- Keep checklist ingest temp uploads in `/var/www/webtest/shared/uploads/openclaw-tmp`
...
1. Copy `infra/nginx/webtest.conf` to `/etc/nginx/sites-available/webtest`
...
4. Configure MariaDB:
   ...
   - create user `webtest_app`
   - grant privileges on that database only
5. Copy `infra/config/shared-config.example.php` to `/var/www/webtest/shared/config.php` and replace placeholders
...
9. Issue or expand TLS with `sudo certbot --nginx --cert-name webtest.solutions -d webtest.solutions -d www.webtest.solutions -d webtest.online`

## Hostname policy

- `https://webtest.solutions` is the canonical backend host
- `https://webtest.online` is legacy and should redirect to `https://webtest.solutions`
...
- `webtest.online` returns a redirect to `https://webtest.solutions/some/path?x=1`
...
- a SQL dump in `/var/backups/webtest`
