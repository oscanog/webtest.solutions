...
$success = "";
$info = "";
$emailValue = webtest_password_reset_session_email();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!webtest_password_reset_verify_csrf($_POST['csrf_token'] ?? '')) {
    ...
    if ($action === 'start_over') {
      ...
      $result = webtest_password_reset_request_otp($conn, $emailValue);
      if ($result['ok']) {
      ...
      $result = webtest_password_reset_resend_otp($conn, $emailValue);
      if ($result['ok']) {
        $success = (string) ($result['message'] ?? '');
      } else {
      ...
      $result = webtest_password_reset_verify_otp($conn, $emailValue, (string) ($_POST['otp'] ?? ''));
      if ($result['ok']) {
        $success = (string) ($result['message'] ?? '');
      } else {
      ...
      if ($result['ok']) {
        header("Location: " . webtest_path('rainier/login.php?reset=success'));
        exit();
      }
...
$currentStep = webtest_password_reset_current_step();
$emailValue = ($emailValue !== '') ? $emailValue : webtest_password_reset_session_email();
$maskedEmail = ($emailValue !== '') ? webtest_password_reset_mask_email($emailValue) : '';
$csrfToken = webtest_password_reset_csrf_token();
$devPreviewOtp = ($currentStep === 'otp') ? webtest_password_reset_dev_preview_otp($emailValue) : '';
$formAction = htmlspecialchars(webtest_path('rainier/forgot_password.php'));
?>
<!DOCTYPE html>
<html lang="en">
